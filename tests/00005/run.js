...
import { clearScreen, printLine, printColoredLine, ANSI_COLOR_YELLOW, ANSI_COLOR_GREEN, ANSI_COLOR_RED } from '../helpers/retro-console.js';
...
This test will register ${NUM_AGENTS} agents.`);
    ...
    for (let i = 0; i < NUM_AGENTS; i++) {
        await printLine("\n-----------------------------------------------------------------");
        await printColoredLine(`Registering Agent ${i + 1} of ${NUM_AGENTS}` , ANSI_COLOR_YELLOW);
        await printLine("-----------------------------------------------------------------");
        ...
        try {
            ...
            await printColoredLine(`${agentPayload.name} ✓`, ANSI_COLOR_GREEN);
        } catch (error) {
            await printColoredLine("--- REGISTRATION FAILED ---", ANSI_COLOR_RED);
